...
public interface TableDao extends GenericDao<Table> {
    ...
    @Query("""
            ...
            FROM `Table` t
            WHERE t.accountId = :accountId
            AND t.status IS 'LOCAL_EDITED'
            AND t.remoteId IS NULL
            """)
    ...
    @Query("""
            SELECT t.remoteId, t.id
            FROM `Table` t
            WHERE t.accountId = :accountId
            AND t.remoteId IN (:remoteIds)
            """)
    Map<
            @MapColumn(columnName = "remoteId") Long,
            @MapColumn(columnName = "id") Long> getTableRemoteAndLocalIds(long accountId, Collection<Long> remoteIds);
