...
public interface ColumnDao extends GenericDao<Column> {
    ...
    @Query("SELECT id, orderWeight FROM `Column` " +
            ...
            "AND status IS NOT 'LOCAL_DELETED' " +
            "ORDER BY orderWeight DESC")
    Map<@MapColumn(columnName = "id") Long, @MapColumn(columnName = "orderWeight") Integer> getNotDeletedOrderWeights(long tableId);

    @Query("SELECT remoteId, id FROM `Column` " +
            "WHERE tableId = :tableId " +
            "AND remoteId IN (:remoteIds)")
    Map<@MapColumn(columnName = "remoteId") Long, @MapColumn(columnName = "id") Long> getColumnRemoteAndLocalIds(long tableId, Collection<Long> remoteIds);
